...
def get_all_pets(current_user):
    try:
        ...
        return jsonify({"status": "success", "data": [format_pet(p) for p in pets]}), 200
...
def update_pet(current_user, pet_id):
    try:
        ...
        """, (
            ...
            data.get('sex'),          data.get('dob'),
            data.get('weight_kg'),    data.get('coat_color'),
            data.get('medical_notes'),data.get('allergies'),
            data.get('is_vaccinated'),data.get('vaccine_record'),
            data.get('behavior_notes'),
            pet_id,
        ))
        ...
        return jsonify({"status": "success", "message": "อัปเดตโปรไฟล์สัตว์เลี้ยงเรียบร้อย"}), 200
